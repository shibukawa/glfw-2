     ...
     replacementRange:(NSRange)replacementRange
{
    ...
    NSString* string = markedText.string;
    ...
    for (i = 0;  i < length;  i++)
    {
        const unichar codepoint = [string characterAtIndex:i];
                         ...
                         actualRange:(NSRangePointer)actualRange
{
    ...
    _glfwPlatformGetWindowPos(window, &x, &y);
    const NSRect contentRect = [window->ns.view frame];
    return NSMakeRect(x + window->preeditCursorPosX, transformY(y+window->preeditCursorPosY), 0, window->preeditCursorHeight);
}
...
int _glfwPlatformGetIMEStatus(_GLFWwindow* window)
{
    ...
    if (asciiSource) {
        NSString* asciiSourceID = (__bridge NSString *)(TISGetInputSourceProperty(asciiSource, kTISPropertyInputSourceID));
        return ([asciiSourceID compare: currentSourceID] == NSOrderedSame) ? GLFW_TRUE : GLFW_FALSE;
    }
    return GLFW_TRUE;
}
