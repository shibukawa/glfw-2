...
//export goPreeditCB
func goPreeditCB(window unsafe.Pointer, textLength C.int, text *C.uint, blockLength C.int, blocks *C.int, focusedBlock int) {
	w := windows.get((*C.GLFWwindow)(window))
	goText := make([]rune, int(textLength))
	var x1 C.uint
	var x2 C.int
	for i := 0; i < int(textLength); i++ {
		p := (*C.uint)(unsafe.Pointer(uintptr(unsafe.Pointer(text)) + uintptr(i)*unsafe.Sizeof(x1)))
		goText[i] = rune(*p)
	}
	goBlocks := make([]int, int(blockLength))
	for i := 0; i < int(blockLength); i++ {
		p := (*C.int)(unsafe.Pointer(uintptr(unsafe.Pointer(blocks)) + uintptr(i)*unsafe.Sizeof(x2)))
		goBlocks[i] = int(*p)
	}
	w.fPreeditHolder(w, goText, goBlocks, int(focusedBlock))
}
...
type PreeditCallback func(w *Window, char []rune, blocks []int, focusedBlock int)

// SetPreeditCallback sets the character callback which is called when a
// IME is handling preedit text.
//
// The character callback is intended for Unicode text input. As it deals with
// characters, it is keyboard layout dependent, whereas the
// key callback is not. Characters do not map 1:1
