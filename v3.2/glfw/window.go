...
type Window struct {
	...
	fPreeditHolder     func(w *Window, text []rune, blocks []int, focusedBlock int)
...
func (w *Window) SetPreeditCursorPos(x, y, h int) {
	C.glfwSetPreeditCursorPos(w.data, C.int(x), C.int(y), C.int(h))
}
...
func (w *Window) GetPreeditCursorPos() (int, int, int) {
	var x, y, h C.int
	C.glfwGetPreeditCursorPos(w.data, &x, &y, &h)
	return int(x), int(y), int(h)
}

// ResetPreeditText resets IME's preedit text.
func (w *Window) ResetPreeditText() {
	C.glfwResetPreeditText(w.data)
}

// GetIMEStatus returns information of IME.
func (w *Window) GetIMEStatus() bool {
	result := C.glfwGetIMEStatus(w.data)
	return result != 0
}

// SetIMEStatus sets IME is active.
func (w *Window) SetIMEStatus(active bool) {
	if active {
		C.glfwSetIMEStatus(w.data, C.int(1))
	} else {
		C.glfwSetIMEStatus(w.data, C.int(0))
	}
}
